...
from gee_database_point_sampler import (
    START_DATE,
    END_DATE,
...
from database_model_definitions import (
    Study,
    Sample,
    Point,
    CovariateDefn,
    ...
    for covariate in covariates:
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                float(value)
                unique_values_per_covariate[covariate.covariate_defn.name].add(
                    True
                )
            except ValueError:
                unique_values_per_covariate[covariate.covariate_defn.name].add(
...
def calculate_display_tables(session, query, max_sample_size):
    ...
    sample_covariate_defns = [
        ...
        if cov_association == CovariateAssociation.SAMPLE.value
        or show_in_point_table == 1
    ]

    study_covariate_defns = [
        (name, always_display, hidden)
        for name, always_display, hidden, cov_association, show_in_point_table, _ in covariate_defns
        if cov_association == CovariateAssociation.STUDY.value
        or show_in_point_table == 1
    ]
    ...
    for sample, study in limited_sample_query:
        ...
        _collect_unique_covariate_values(
            sample_covariates, unique_sample_covariate_values
        )
    ...
    for study in study_list:
        ...
        _collect_unique_covariate_values(
            covariates, unique_study_covariate_values
        )
    ...
    for sample, covariate_list in sample_covariate_list:
        ...
        display_row = [
            covariate_dict.get(name) for name in sample_covariate_display_order
        ]
    ...
    for study, covariate_list in study_covariate_list:
        ...
        display_row = [
            covariate_dict.get(name) for name in study_covariate_display_order
        ]
...
def initialize_covariates(clear_cache):
    ...
    pkcl_filepath = os.path.join(
        INSTANCE_DIR, "initialize_searchable_covariates.pkl"
    )
    ...
    searchable_unique_covariates = (
        session.query(CovariateDefn.name, CovariateValue.value)
        .filter(CovariateDefn.queryable, CovariateDefn.search_by_unique == True)
        .join(CovariateValue)
    ).yield_per(1000)
    ...
    searchable_continuous_covariates = session.query(
    ...
    for index, row in enumerate(searchable_continuous_covariates):
        ...
        COVARIATE_STATE["searchable_covariates"][row.name] = CovariateType(
            row.covariate_type
        ).name
    ...
    COVARIATE_STATE["country_set"] = [
        x[0]
        for x in session.query(distinct(Geolocation.geolocation_name))
        .filter(Geolocation.geolocation_type == "COUNTRY")
        .all()
    ]
    COVARIATE_STATE["continent_set"] = [
        x[0]
        for x in session.query(distinct(Geolocation.geolocation_name))
        .filter(Geolocation.geolocation_type == "CONTINENT")
        .all()
    ]

    # add the study ids manually
    COVARIATE_STATE["searchable_covariates"][STUDY_ID] = [
        x[0] for x in session.query(distinct(Study.name)).all()
    ]
    ...
    covariate_defns = (
        ...
        .order_by(CovariateDefn.display_order, func.lower(CovariateDefn.name))
        .all()
    )
    ...
    for key in COVARIATE_STATE["searchable_covariates"]:
        COVARIATE_STATE["searchable_covariates"][key] = sorted(
            COVARIATE_STATE["searchable_covariates"][key]
        )
...
def form_to_dict(form):
    ...
    return {
        ...
        "centerPointBuffer": (
            None if not centerPointBuffer else centerPointBuffer
        ),
        ...
        "sampleSizeMinObservationsPerYear": int(
            form.get("sampleSizeMinObservationsPerYear")
        ),
...
def build_filter(session, form):
    ...
    covariate_defns = (
        session.query(
            CovariateDefn.name,
            CovariateDefn.id_key,
            CovariateDefn.covariate_association,
        )
        .filter(CovariateDefn.name.in_(fields))
        .all()
    )
    covariate_defn_map = {
        name: (id_key, association)
        for name, id_key, association in covariate_defns
    }

    for field, values in zip(fields, values_list):
        if not field or not values:
            continue
        covariate_defn_id, covariate_association = covariate_defn_map.get(
            field, (None, None)
        )
        filter_text += (
            f"{field}({covariate_association}) = " + "|".join(values) + "\n"
        )
        if field == STUDY_ID:
        ...
        if covariate_association == CovariateAssociation.STUDY.value:
            study_ids_subq = covariate_subquery.with_entities(
                CovariateValue.study_id
            ).subquery()
            filters.append(Study.id_key.in_(study_ids_subq))
        elif covariate_association == CovariateAssociation.SAMPLE.value:
            sample_ids_subq = covariate_subquery.with_entities(
                CovariateValue.sample_id
            ).subquery()
    ...
    if center_point is not None:
        ...
        if center_point != "":
            ...
            center_point_buffer = float(form["centerPointBuffer"].strip()) / 2
            ...
            filter_text += (
                f"center point at ({lat},{lng}) + +/-{center_point_buffer}\n"
            )
    ...
    if upper_left_point is not None:
        ...
        if upper_left_point != "":
            ...
            filter_text += (
                f"bounding box ({ul_lat},{ul_lng}) ({lr_lat},{lr_lng})\n"
            )
    ...
    if min_sites_per_study:
        ...
        min_sites_per_study_subquery = (
            session.query(
                Study.id_key, func.count(func.distinct(Sample.point_id))
            )
            .join(Study.samples)
        ...
        filters.append(Study.id_key == min_sites_per_study_subquery.c.id_key)
    ...
    if sample_size_min_years > 0:
        ...
        valid_study_ids_subquery = (
            ...
            .having(
                func.count(func.distinct(Sample.year)) >= sample_size_min_years
            )
            .subquery()
        )
    ...
    if min_observations_per_year > 0:
        filter_text += (
            f"min observations per year {min_observations_per_year}\n"
        )
        ...
        valid_study_ids_subquery = (
            ...
            .having(
                func.min(counts_per_study_year.c.samples_per_year)
    ...
    if year_range:
        ...
        year_subquery_sample_ids = (
            ...
            .filter(Sample.year.in_(year_set))
            .subquery()
        )
        filters.append(Sample.id_key.in_(year_subquery_sample_ids))
...
def get_data():
    ...
    if sent_columns:
        ...
        sent_sample_columns = {
            name: order for name, order in sent_sample_columns
        }
    ...
    for sample, study in samples_chunk:
        ...
        # Collect point data
        points.append(
            {"lat": sample.point.latitude, "lng": sample.point.longitude}
        )
    ...
    sorted_sample_columns = sorted(
        sent_sample_columns.items(), key=lambda x: x[1]
    )
    sorted_study_columns = sorted(
        sent_study_columns.items(), key=lambda x: x[1]
    )

    # Store back in Redis
    redis_client.set(
        sent_columns_key,
        json.dumps([sorted_sample_columns, sorted_study_columns]),
    )
...
def view_covariate():
    return render_template(
        "covariate_view.html",
        covariate_association_states=[
            str(x).split(".")[1] for x in CovariateAssociation
        ],
    )
    ...
    covariates = [
        {
            ...
            "covariate_association": CovariateAssociation(
                c.covariate_association
            ).value,
            "hidden": c.hidden,
...
def update_covariate():
    ...
    for remote_covariate in data["covariates"]:
        local_covariate = session.query(CovariateDefn).get(
            remote_covariate["id_key"]
        )
...
def _prep_download(self, query_id):
    ...
    try:
        ...
        sample_covariate_names = list(
            name
            for (name,) in session.query(CovariateDefn.name)
            .join(
                CovariateValue,
                CovariateDefn.id_key == CovariateValue.covariate_defn_id,
            )
            ...
            .order_by(
                CovariateDefn.display_order, func.lower(CovariateDefn.name)
            )
            .distinct()
        )
        ...
        if batch_rows:
            ...
            df.fillna("None", inplace=True)
            df.to_csv(
                sample_csv_path, mode="a", header=first_batch, index=False
            )
        ...
        for study in (
            session.query(Study)
            .filter(Study.id_key.in_(study_ids))
        ...
        df.fillna("None", inplace=True)
        df.to_csv(study_csv_path, mode="a", header=True, index=False)
        ...
        return {"query_id": query_id, "zipfile_path": zipfile_path}
        ...
        for temp_file in temp_files:
            if os.path.exists(temp_file):
                try:
                    ...
                    LOGGER.exception(
                        f"Error deleting temporary file: {temp_file}"
                    )
...
def data_extractor():
    if request.method == "POST":
        ...
        task = gee_data_pull_task.delay(
            form_data, csv_content, csv_file.filename
        )
...
def download_csv_template():
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=eo_sample_template.csv"
    )

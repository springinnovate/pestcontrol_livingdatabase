        ...
        if url.lower().endswith(".pdf"):
            ...
            async with domain_semaphore, browser_semaphore:
                ...
                try:
                    ...
                    await page.goto(
                        url, timeout=10000
                    )  # maybe a longer timeout for safety
                    LOGGER.debug("getting content")
                    raw_content = await page.content()
                    content = "\\n".join(
                        [
                            x.strip()
                            for x in extract_text_elements(raw_content)
                            if x.strip()
                        ]
                    )
                    ...
                    LOGGER.exception(
                        f"something terrible happened in fetch page content"
                    )
                    raise
                finally:
                    await page.close()
...
async def handle_question_species(
    row, species, openai_context, openai_semaphore, browser_semaphore, context
):
    try:
        ...
        aggregate_answer = await aggregate_answers(
...
async def make_request_with_backoff(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            LOGGER.exception(
                f"attempt {attempt} failed on openai exception {e} on {chat_args}"
            )
...
async def generate_text(
    ...
    if key in PROMPT_CACHE:
        ...
        LOGGER.info(
            f'NOT CACHED openai message submitted: {messages[0]["content"][5000:]}'
        )
...
async def process_one_search_result(
    ...
    return text_content, await get_webpage_answers(
        openai_semaphore,
        openai_context,
        text_content,
        question,
